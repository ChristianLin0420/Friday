...
class TrainingRecordViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private let Data = ColumnsData()
    private let Record = CharacterRecord()
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.async {
            self.RecordDetails = self.Data.GetTrainingRecordDetail()
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return Data.GetCurrentCoreDataCount()
    }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            Data.DeleteColumnsData(index: indexPath.row)
            Record.DeleteCharactersRecord(index: indexPath.row)
            TrainingRecordTable.reloadData()
            if Data.GetCurrentCoreDataCount() == 0 {
